...
class PrecisionMissionController(Node):
    def __init__(self):
        super().__init__('precision_mission_controller')
    ...
    def get_target_or_hold(self, now, current_x, current_y, current_z):
        if self.mission_state != "TRACK_TARGET":
            return self.get_hold_target(
                "HOLD_IDLE",
                current_x,
                current_y,
                current_z
        ...
        if self.target_position is None or self.last_target_time is None:
            return self.get_hold_target(
                "HOLD_NO_TARGET",
                current_x,
                current_y,
        ...
        if not self.use_target_z:
            target_z = self.safe_target_z
        ...
        if target_distance > self.max_target_distance:
            self.get_logger().warn(
                f"Rejecting target: distance={target_distance:.2f} m "
                f"> max_target_distance={self.max_target_distance:.2f} m, "
                f"target=({target_x:.2f}, {target_y:.2f}, {target_z:.2f}), "
                f"current=({current_x:.2f}, {current_y:.2f}, {current_z:.2f})"
            )
            return self.get_hold_target(
                "HOLD_TARGET_REJECTED",
                current_x,
                current_y,
                current_z
            )

        return (target_x, target_y, target_z), "TRACK_TARGET"



    def get_hold_target(self, mode, current_x, current_y, current_z):
        if self.hold_mode != mode:
            self.hold_mode = mode
            self.hold_x = current_x
            self.hold_y = current_y
            self.hold_z = current_z
            self.get_logger().info(
                f"Entering {mode}: hold=({self.hold_x:.2f}, {self.hold_y:.2f}, {self.hold_z:.2f})"
            )

        return (self.hold_x, self.hold_y, self.hold_z), mode
    ...
    def timer_callback(self):
        ...
        # target topic 기반 target 생성
        target, target_mode = self.get_target_or_hold(
            now,
            current_x,
            current_y,
            current_z
        ...
        if target_mode == "TRACK_TARGET":
            if dist < self.acceptance_radius:
                if self.target_reached_time is None:
                    self.target_reached_time = now
                    self.target_reached_logged = True
                    self.get_logger().info(
                        f"Target entered acceptance radius "
                        f"({dist:.2f} m < {self.acceptance_radius:.2f} m). Holding."
                    )
            else:
                self.target_reached_time = None
                self.target_reached_logged = False
...
def main(args=None):
    ...
    node = PrecisionMissionController()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()

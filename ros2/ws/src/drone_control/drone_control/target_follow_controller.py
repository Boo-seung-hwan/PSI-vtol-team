...
class TargetFollowController(Node):
    def __init__(self):
    ...
    def get_target_or_hold(self, now, current_x, current_y, current_z):
        if self.target_position is None or self.last_target_time is None:
            return (current_x, current_y, current_z), "HOLD_NO_TARGET"

        if now - self.last_target_time > self.target_timeout:
            return (current_x, current_y, current_z), "HOLD_TARGET_TIMEOUT"

        target_x, target_y, target_z = self.target_position

        # 초기 테스트에서는 target의 z를 그대로 쓰지 않고 안전 고도 유지 추천
        # self.use_target_z = False이면 x, y만 따라가고 z는 safe_target_z 사용
